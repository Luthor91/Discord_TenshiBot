...
func GetExperienceCommand(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if strings.HasPrefix(m.Content, command) {
		exp, exists := services.NewUserService(controllers.NewUserController()).GetExperience(m.Author.ID)
		if !exists {
